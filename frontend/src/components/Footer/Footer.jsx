...
const Footer = () => {
  return (
    <footer className="relative bg-white pt-24 pb-12 overflow-hidden">
      ...
      {/* Animated Flag Mesh Gradient Background */}
      <div className="absolute inset-0 -z-10 bg-[#f8fafc]">
        <div className="absolute top-[-10%] right-[-10%] w-[500px] h-[500px] bg-[#FF9933]/10 rounded-full blur-[120px] animate-blob"></div>
        <div className="absolute bottom-[-10%] left-[-10%] w-[500px] h-[500px] bg-[#138808]/10 rounded-full blur-[120px] animate-blob animation-delay-2000"></div>
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-white rounded-full blur-[150px] animate-blob animation-delay-4000"></div>
      </div>

      {/* Floating Medical-Themed Decorative Shapes */}
      <div className="absolute top-20 left-10 opacity-10 animate-float">
        <svg width="40" height="40" viewBox="0 0 24 24" fill="currentColor" className="text-[#FF9933]">
          <path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-2 10h-4v4h-2v-4H7v-2h4V7h2v4h4v2z" />
        </svg>
      </div>
      <div className="absolute bottom-40 right-20 opacity-10 animate-float-delayed">
        <svg width="60" height="60" viewBox="0 0 24 24" fill="currentColor" className="text-[#138808]">
      ...
      <div className="container mx-auto px-6 lg:px-16 relative">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-20">
            ...
            <ul className="space-y-5">
              {aboutLinks.map((link) => (
                <li key={link.display}>
                  <Link
                    to={link.path}
                    className="group flex items-center text-[16px] font-black text-gray-400 hover:text-[#138808] transition-all duration-300"
                  >
                    <div className="w-1.5 h-1.5 rounded-full bg-[#FF9933] mr-0 opacity-0 transition-all duration-300 group-hover:mr-4 group-hover:opacity-100 group-hover:scale-125"></div>
                    <span className="relative overflow-hidden uppercase tracking-tighter">
                      {link.display}
                      <span className="absolute bottom-0 left-0 w-full h-[2px] bg-[#138808] -translate-x-full transition-transform duration-300 group-hover:translate-x-0"></span>
                    </span>
                  </Link>
                </li>
              ))}
          ...
          <div className="lg:pl-10">
            <h3 className="text-2xl font-black mb-10 text-gray-800 relative group/title inline-block">
              Services
              <span className="absolute -bottom-3 left-0 w-16 h-1.5 bg-gradient-to-r from-[#FF9933] to-transparent rounded-full transition-all duration-500 group-hover/title:w-full"></span>
            </h3>
            <ul className="space-y-5">
            ...
            <ul className="space-y-8">
              <li className="flex items-center gap-5 group cursor-pointer">
                ...
                <div className="flex flex-col">
                  <span className="text-[14px] uppercase tracking-widest text-[#FF9933] font-black">Location</span>
                  <span className="text-[15px] font-black text-gray-800 group-hover:text-gray-900 tracking-tighter">Bara Bazar Madhubani, Bihar</span>
                </div>
              </li>
              <li className="flex items-center gap-5 group cursor-pointer">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-teal-50 to-white border border-teal-100 flex items-center justify-center shadow-lg group-hover:from-[#2eb8a6] group-hover:to-[#1d7a6e] group-hover:text-white group-hover:rotate-12 transition-all duration-500">
        ...
        <div className="pt-12 border-t border-teal-200/40 flex flex-col md:flex-row justify-between items-center gap-8 bg-white/20 backdrop-blur-sm p-8 rounded-[40px] shadow-inner mb-4">
          <div className="flex items-center space-x-8">
            {socialLinks.map((socialLink) => (
              <Link
                key={socialLink.id}
                to={socialLink.path}
                className="w-14 h-14 rounded-2xl tri-glass flex items-center justify-center text-gray-400 trasition-all duration-500 hover:-translate-y-4 hover:scale-110 group relative"
              >
                <span className="transition-transform duration-500 group-hover:rotate-[360deg] group-hover:text-orange-600">
                  {socialLink.icon}
                </span>
              </Link>
            ))}
          ...
          <div className="flex flex-col items-center md:items-end">
            ...
            <p className="text-gray-500 text-[14px] font-medium tracking-wide">
              &copy; {new Date().getFullYear()} <span className="font-black"><span style={{ color: "#FF9933" }}>Sehaat</span> <span style={{ color: "#138808" }}>Saathi</span></span>. Crafted with Excellence.
            </p>
          </div>
        </div>
      </div>
